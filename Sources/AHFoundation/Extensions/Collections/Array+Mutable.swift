...
public extension Array {
    ...
    func moving(from index: Index, to newIndex: Index) -> Array {
        AHFoundation.transform(self) { $0.move(from: index, to: newIndex) }
    }
    ...
    func appending(_ element: Element) -> [Element] {
        AHFoundation.transform(self) { $0.append(element) }
    }
    ...
    func removing(at index: Index) -> [Element] {
        AHFoundation.transform(self) { $0.remove(at: index) }
    }
}
...
public extension Array where Element: Equatable {
    ...
    func removing(_ element: Element) -> [Element] {
        AHFoundation.transform(self) { $0.removeFirst(element) }
    }
